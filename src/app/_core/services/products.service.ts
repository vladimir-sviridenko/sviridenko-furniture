...
export class ProductsService {
  ...
  public currentProduct$: BehaviorSubject<Product> = new BehaviorSubject<Product>(undefined);

  public isLoading: boolean = true;
  public currentAlbumId: number;
  public productCards$: BehaviorSubject<Product[]> = new BehaviorSubject<Product[]>(undefined);
  ...
  public updateProductCards(albumId: number): Product[] {
    ...
    const productCards: Product[] = serviceWithAlbum.getProductCards(currentAlbum);
    this.productCards$.next(productCards);
    return productCards;
  }

  public updateProduct(albumId: number, productId: number): Product {
    const serviceWithAlbum: ProductService = this.getServiceBy(albumId);
    const productCard: Product = serviceWithAlbum && (serviceWithAlbum !== this.galleryService)
